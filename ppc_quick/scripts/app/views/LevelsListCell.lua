...
function LevelsListCell:ctor(size, beginLevelIndex, endLevelIndex, rows, cols)
    ...
    for row = 1, rows do
        ...
        for column = 1, cols do
            local icon = display.newSprite("#icon_nor_1.png", x, y)
            batch:addChild(icon)
    ...
    self.highlightButton = display.newSprite("#playing_highscore.png")
...
function LevelsListCell:onTap(x, y)
    local button = self:checkButton(x, y)
    if button then
        self:dispatchEvent({name = "onTapLevelIcon1", levelIndex = button.levelIndex})
    end
end

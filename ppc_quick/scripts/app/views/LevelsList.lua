...
function LevelsList:ctor(rect)
    ...
    for pageIndex = 1, numPages do
        ...
        echoInfo("cell:addEventListener START")
        -- cell:addEventListener("onTapLevelIcon1", function(event) return self:onTapLevelIcon1(event) end)
        cell:addEventListener("onTapLevelIcon1", handler(self, self.onTapLevelIcon1))
        echoInfo("cell:addEventListener END")
        self:addCell(cell)
...
function LevelsList:onTapLevelIcon1(event)
    self:dispatchEvent({name = "onTapLevelIcon2", levelIndex = event.levelIndex})
end

...
local PlayLevelScene = class("PlayLevelScene", function()
    echoInfo("class PlayLevelScene -se")
    return display.newScene("PlayLevelScene")
end)

function PlayLevelScene:ctor(levelIndex)
    echoInfo("PlayLevelScene:ctor -s")

    local bg = display.newSprite("#bg_playing.png")
    -- make background sprite always align top
    bg:setPosition(display.cx, display.top - bg:getContentSize().height / 2)
    self:addChild(bg)
    echoInfo("PlayLevelScene:ctor -e")
    ...
    -- local title = display.newSprite("#icon_dis_0.png", display.left + 150, display.top - 50)
    -- title:setScale(0.5)
    -- self:addChild(title)

    -- local title1 = display.newSprite("#Title.png", display.left + 150, display.top - 100)
    -- title:setScale(0.5)
...
function PlayLevelScene:onLevelCompleted()
    ...
    local dialog = display.newSprite("#icon_dis_3.png")
    dialog:setPosition(display.cx, display.top + dialog:getContentSize().height / 2 + 40)
    self:addChild(dialog)
    if CCUserDefault:sharedUserDefault():getBoolForKey("bool") == false then
